"""
Simulate the Change Stream Worker with dummy data.
...
Usage:
    python -m tests.test_worker_simulate
"""
...
def main():
    ...
    try:
        ...
        print(f"  NER API:  {deps.get('ner_api', '?')}")
        print(f"  NEL API:  {deps.get('nel_api', '?')}")
    ...
    for i, job in enumerate(DUMMY_JOBS, 1):
        ...
        if fp in results_store and results_store[fp]["input_text_hash"] == input_hash:
            print("  SKIP — already classified with same input hash")
            continue

        print("  Calling Classify API...")
        start = time.time()
        try:
            resp = requests.post(
        ...
        for ent in entities[:5]:
            ...
            top_match = (
                f" -> {linked[0]['label']} ({linked[0]['similarity_score']})"
                if linked else ""
            )
        ...
        print("  SAVED to classified-jobs (in-memory)")
    ...
    input_hash = compute_hash(input_text)
    if fp in results_store and results_store[fp]["input_text_hash"] == input_hash:
        print(f"  SKIP — {fp} already classified with same input hash (ok)")
    else:
        print("  ERROR — dedup failed!")
    ...
    first_result = list(results_store.values())[0]
    print(f"  NER model        : {first_result['metadata'].get('model_name', '?')}")
    print(f"  NEL model        : {first_result['metadata'].get('linker_model', '?')}")
    print(f"\nFull pipeline: Scraper -> raw-jobs -> [Worker] -> Classify API -> classified-jobs")
