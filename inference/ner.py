...
class NERModel:
    ...
    def extract(self, text: str) -> List[dict]:
        """Extract entities from text, returning entity_type, surface_form, and span for each."""
        text = text.replace("\n", " ")
    ...
    def _ner_pipeline(self, text: str) -> List[dict]:
        """Run NER on a single sentence, reusing EntityLinker's static helpers."""
        ...
        predicted_tags = EntityLinker.fix_bio_tags(predicted_tags)

        input_ids, predicted_tags = EntityLinker.remove_special_tokens_and_tags(
            inputs["input_ids"][0], predicted_tags, self.tokenizer
        )

        result = EntityLinker.extract_entities(input_ids, predicted_tags)
