...
class NELLinker:
    ...
    def link(self, entities: List[dict], top_k: Optional[int] = None, min_similarity: float = 0.0) -> List[dict]:
        """Link entities to ESCO taxonomy entries. Each entity dict needs 'text' and 'entity_type'."""
        k = top_k or self.k
        results = []
    ...
    def _top_k(self, embedding: torch.Tensor, entity_type: str, k: int, min_similarity: float) -> List[dict]:
        ...
        matches = []
        for idx, score in zip(top_k_results.indices.tolist(), top_k_results.values.tolist()):
