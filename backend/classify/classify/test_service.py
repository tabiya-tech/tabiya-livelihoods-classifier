...
def _make_nel_response(linked_entities: list[dict] | None = None) -> dict:
    if linked_entities is None:
        linked_entities = [
            ...
            {
                "input_text": "plan menus",
                "entity_type": "skill",
                "matches": [{"similarity_score": 0.8, "taxonomy": "esco", "label": "menu planning"}],
            },
        ]
    return {
...
class TestClassifyService:
    class TestClassify:
        ...
        async def test_classify_successful(self):
            ...
            # THEN two entities are returned, each with its own NEL alignment (by NER order)
            assert len(result.classification.entities) == 2
            assert result.classification.entity_counts == {"occupation": 1, "skill": 1}
            assert result.classification.entities[0].linked_entities[0]["label"] == "chef"
            assert result.classification.entities[1].linked_entities[0]["label"] == "menu planning"
        ...
        @pytest.mark.asyncio
        async def test_classify_duplicate_surface_each_gets_distinct_nel_matches(self):
            # GIVEN two skill spans with the same surface_form at different positions
            mock_ner = MockNERClient()
            mock_ner.extract = AsyncMock(
                return_value=_make_ner_response(
                    entities=[
                        {
                            "entity_type": "skill",
                            "surface_form": "Python",
            ...
            mock_nel.link = AsyncMock(
                return_value=_make_nel_response(
                    linked_entities=[
                        {
                            "input_text": "Python",
                            "entity_type": "skill",
                            "matches": [{"similarity_score": 0.9, "taxonomy": "esco", "label": "python first"}],
                        },
                        {
                            "input_text": "Python",
                            "entity_type": "skill",
                            "matches": [{"similarity_score": 0.85, "taxonomy": "esco", "label": "python second"}],
                        },
                    ]
                )
            )
            ...
            # WHEN classifying
            result = await service.classify("Python ... later Python")

            # THEN each entity keeps its own linked_entities (no overwrite by surface_form key)
            assert len(result.classification.entities) == 2
            m0 = result.classification.entities[0].linked_entities
            m1 = result.classification.entities[1].linked_entities
            assert m0 is not None and m1 is not None
            assert m0[0]["label"] == "python first"
            assert m1[0]["label"] == "python second"
        ...
        @pytest.mark.asyncio
        async def test_classify_passes_experience_entity_type_to_ner(self):
            mock_ner = MockNERClient()
            mock_ner.extract = AsyncMock(return_value=_make_ner_response(entities=[]))
            mock_nel = MockNELClient()
            mock_nel.link = AsyncMock()
            ...
            options = ClassifyOptions(extract_entities=["experience"])
            await service.classify("some text", options=options)

            mock_ner.extract.assert_called_once_with("some text", ["experience"])
